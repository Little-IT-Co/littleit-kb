# Knowledge Base

Welcome to the **Little Cloud Knowledge Base**! Here you'll find comprehensive guides and documentation for our supported services.

## Available Services

### Google Workspace

Complete documentation for Google Workspace services including email, collaboration tools, and administrative controls.

- [Google Workspace Overview](/kb/google-workspace/) - Get started with Google Workspace
- [Gmail](/kb/google-workspace/gmail) - Professional email solution
- [Google Admin](/kb/google-workspace/google-admin) - Administrative controls and management
- [Google Drive, Docs, Sheets and Slides](/kb/google-workspace/google-drive-docs-sheets-slides) - File storage and productivity suite
- [Google Meet](/kb/google-workspace/google-meet) - Video conferencing and meetings
- [Google Keep](/kb/google-workspace/google-keep) - Note-taking and organization

### Microsoft 365

Comprehensive guides for Microsoft 365 services including email, collaboration, and identity management.

- [Microsoft 365 Overview](/kb/microsoft-365/) - Get started with Microsoft 365
- [Outlook & Exchange](/kb/microsoft-365/outlook-exchange/) - Email and calendaring
  - [The Web App vs The Client](/kb/microsoft-365/outlook-exchange/web-app-vs-client) - Understanding different access methods
  - [Mailboxes](/kb/microsoft-365/outlook-exchange/mailboxes) - Managing mailboxes and storage
- [Adding Users](/kb/microsoft-365/adding-users) - User account management
- [Adding Licences](/kb/microsoft-365/adding-licences) - License management
- [OneDrive & SharePoint](/kb/microsoft-365/onedrive-sharepoint) - File storage and collaboration
- [Microsoft Teams](/kb/microsoft-365/microsoft-teams) - Team collaboration and meetings
- [Microsoft Entra](/kb/microsoft-365/microsoft-entra) - Identity and access management

### Other Services

Documentation for additional services and tools we support.

- [WordPress](/kb/wordpress/) - Content management system
  - [Plugins](/kb/wordpress/plugins/) - Essential plugins and configurations
    - [Google Site Kit](/kb/wordpress/plugins/google-site-kit) - Google services integration
    - [Elementor](/kb/wordpress/plugins/elementor) - Page builder
    - [Limit Login Attempts](/kb/wordpress/plugins/limit-login-attempts) - Security plugin
    - [WP Mail Options](/kb/wordpress/plugins/wp-mail-options) - Email configuration
  - [Best Practices for Security](/kb/wordpress/security-best-practices) - Security guidelines
  - [Installation Guides](/kb/wordpress/installation/) - Installing WordPress
    - [Installing on cPanel](/kb/wordpress/installation/cpanel) - Using cPanel installer
    - [Installing on Docker](/kb/wordpress/installation/docker) - Containerized installation
    - [Installing on Coolify](/kb/wordpress/installation/coolify) - Deploying with Coolify
- [cPanel](/kb/cpanel) - Web hosting control panel
- [CyberPanel](/kb/cyberpanel) - LiteSpeed control panel
- [Cloudmin](/kb/cloudmin) - Virtual machine management
- [Docker](/kb/docker) - Containerization platform
- [GitHub](/kb/github) - Version control and collaboration

## Getting Help

Our documentation is designed to help you get the most out of your cloud services. If you need additional assistance:

- **Contact Support** - Reach out to our [24/7 support team](https://little.cloud/contact)
- **Search** - Use the search function to find specific topics
- **Browse by Category** - Explore services by category in the sidebar

---

*Need help? Contact our [24/7 support team](https://little.cloud/contact)*

